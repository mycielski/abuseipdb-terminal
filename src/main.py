...
from bs4 import (
    BeautifulSoup,
)  # to extract meaningful data from response easily and quickly

parser = argparse.ArgumentParser(description="Check IP addresses in AbuseIPDB.")
parser.add_argument(
    "addresses", metavar="ip", type=str, nargs="+", help="an address to check"
)
...
for address in addresses_validated:
    ...
    soup = BeautifulSoup(
        r.text, "html.parser"
    )  # for increased speed download and use "lxml.parser"
